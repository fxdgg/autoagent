# Task 8.2: Push container image — Round 1.2
...
<instructions>
    ...
    2. If you detect a prerequisite blocker and needs a specialized analysis task to diagnose, end your response with EXACTLY:
      ❌ FATAL: <reason>
    Prioritize user-defined completion marker rules in `initial_hint` and `completion_criteria` section over this rule.
    3. For any command that may run longer than a few minutes,
    ...
    Check the command outputs in redirected files or "logs/<SESSION>/lr_tasks/lr_8.2_output.log" and continue working if it has already finished.
    DO NOT use `sleep` or any wait command in your session.
    
    4. When you are done, end your response with EXACTLY one of:
      ✅ completed
...
<guidance_from_previous_failure>
    Refresh the registry credentials before pushing. Use a longer-lived token or service account credentials instead of short-lived user tokens.
